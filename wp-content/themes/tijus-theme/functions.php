...
/**
 * Enable user registration and explicitly brand the default WP signin UI.
 */
...
function tijus_custom_login_ui() {
    wp_enqueue_style( 'tijus-custom-login', get_template_directory_uri() . '/assets/css/custom-login.css', array(), '1.1.0' );
    
    // Inject custom HTML elements via JS to avoid modifying core forms if hooks aren't enough
    // Actually, we can use hooks for most parts.
}
add_action( 'login_enqueue_scripts', 'tijus_custom_login_ui' );

function tijus_login_header_content() {
    ?>
...
add_action( 'login_message', 'tijus_login_header_content' );

function tijus_login_form_trouble_link() {
    echo '<a href="' . esc_url( wp_lostpassword_url() ) . '" class="trouble-link">Having trouble in sign in?</a>';
}
add_action( 'login_form', 'tijus_login_form_trouble_link' );

function tijus_login_footer_content() {
    ?>
    ...
    <div class="social-buttons">
        ...
        <a href="#" class="social-btn">
            <svg width="20" height="20" viewBox="0 0 320 512" fill="#1877F2"><path d="M279.14 288l14.22-92.66h-88.91v-60.13c0-25.35 12.42-50.06 52.24-50.06h40.42V6.26S260.43 0 225.36 0c-73.22 0-121.08 44.38-121.08 124.72v70.62H22.89V288h81.39v224h100.17V288z"/></svg> Facebook
        </a>
    </div>
    <div class="login-footer-link">
        Don't have an account? <a href="<?php echo esc_url( wp_registration_url() ); ?>">Request Now</a>
    </div>
    <div class="login-copyright">
        Copyright @wework 2022 <span>|</span> Privacy Policy
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Move our trouble link before the submit button
...
add_action( 'login_footer', 'tijus_login_footer_content' );

function tijus_custom_login_url() {
    return home_url();
}
add_filter( 'login_headerurl', 'tijus_custom_login_url' );
